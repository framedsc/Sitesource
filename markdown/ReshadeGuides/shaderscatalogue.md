...
- [**ReVeil**](https://github.com/LordOfLunacy/Insane-Shaders/blob/master/Shaders/ReVeil.fx): Similar to the dehaze option in photoshop, except that it also allows you to reintroduce the haze afterward. Super useful with ambient occlusion or raytracing effects to help them look native. Put the shaders that you don't want to see on top of smoke/fog/mist between the `Reveil_Top` and `Reveil_Bottom` techniques. RadiantGI has native reveil support so it doesn't need it.
...
- [**Volumetric Fog V2**](https://github.com/mj-ehsan/NiceGuy-Shaders/blob/main/Shaders/Volumetric%20Fog%20V2.0.fx)
